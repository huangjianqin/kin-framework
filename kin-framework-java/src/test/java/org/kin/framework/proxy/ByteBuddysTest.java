...
public class ByteBuddysTest {
    public static void main(String[] args) {
        ...
        ByteBuddyFactory byteBuddyFactory = Proxys.byteBuddy();

        AddService addService = new AddService();

        Method targetMethod = AddService.class.getMethods()[0];
        System.out.println(targetMethod);
        ProxyInvoker<AddService> proxyInvoker = byteBuddyFactory.enhanceMethod(new MethodDefinition<>(addService, targetMethod));
        try {
        ...
        System.out.println("--------------");
        AddService addServiceProxy = byteBuddyFactory.enhanceClass(new ClassDefinition<>(addService));
