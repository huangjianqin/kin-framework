...
final class JavassistTypeParameterMatcherGenerator {
    private static final Logger log = LoggerFactory.getLogger(JavassistTypeParameterMatcherGenerator.class);
    ...
    public static TypeParameterMatcher generate(Class<?> type) {
        ClassLoader classLoader = SysUtils.getContextClassLoader();
        if (classLoader == null) {
            classLoader = SysUtils.getSystemClassLoader();
        }
    ...
    public static TypeParameterMatcher generate(Class<?> type, ClassLoader classLoader) {
        ...
        try {
            ...
            if (type != Object.class) {
                log.debug("Generated: {}", generated.getName());
            } else {
