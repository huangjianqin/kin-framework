...
/**
 * {@link SingleThreadEventExecutor}池
 *
 * @author huangjianqin
 * @date 2020/11/23
 */
public class SingleThreadEventExecutorPool implements ScheduledExecutorService {
    //状态枚举
    ...
    /** 原子更新状态值 */
    private static final AtomicIntegerFieldUpdater<SingleThreadEventExecutorPool> STATE_UPDATER =
            AtomicIntegerFieldUpdater.newUpdater(SingleThreadEventExecutorPool.class, "state");
    ...
    private final SingleThreadEventExecutor[] executors;

    public SingleThreadEventExecutorPool(int coreSize) {
        this(coreSize, StringUtils.firstLowerCase(SingleThreadEventExecutorPool.class.getSimpleName()));
    }
    ...
    public SingleThreadEventExecutorPool(int coreSize, String workerNamePrefix) {
        ...
        this.executors = new SingleThreadEventExecutor[coreSize];
        for (int i = 0; i < coreSize; i++) {
            executors[i] = new SingleThreadEventExecutor(executionContext);
        }
    }
     ...
     * 选择一个executor
     */
    private SingleThreadEventExecutor selectExecutor(Object task) {
    ...
    public void shutdown() {
        for (SingleThreadEventExecutor executor : executors) {
            executor.shutdown();
        }
