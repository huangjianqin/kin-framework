...
abstract class TypeParameterMatcher {
    private static final Logger log = LoggerFactory.getLogger(TypeParameterMatcher.class);

    private static final TypeParameterMatcher NOOP = new NoOpTypeParameterMatcher();
    private static final Object TEST_OBJECT = new Object();

    private static boolean hasJavassist() {
        boolean noJavassist = SysUtils.getBoolSysProperty("org.kin.framework.noJavassist", false);
        log.debug("-Dorg.kin.framework.noJavassist: {}", noJavassist);

        if (noJavassist) {
            log.debug("Javassist: unavailable (org.kin.framework.noJavassist)");
            return false;
        }

        try {
            JavassistTypeParameterMatcherGenerator.generate(Object.class, SysUtils.getClassLoader(TypeParameterMatcher.class));
            log.debug("Javassist: available");
            return true;
        } catch (Throwable t) {
            // Failed to generate a Javassist-based matcher.
            log.debug("Javassist: unavailable");
            log.debug(
